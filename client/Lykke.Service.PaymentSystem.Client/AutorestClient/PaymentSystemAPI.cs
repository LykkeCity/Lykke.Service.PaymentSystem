...
namespace Lykke.Service.PaymentSystem.Client.AutorestClient
{
    ...
    public partial class PaymentSystemAPI : ServiceClient<PaymentSystemAPI>, IPaymentSystemAPI
    {
        ...
        /// A response object containing the response body and response headers.
        /// </return>
        public async Task<HttpOperationResponse<object>> GetSourceClientIdWithHttpMessagesAsync(string walletId = default(string), string clientPaymentSystem = default(string), Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            // Tracing
            bool _shouldTrace = ServiceClientTracing.IsEnabled;
            string _invocationId = null;
            if (_shouldTrace)
            {
                ...
                tracingParameters.Add("cancellationToken", cancellationToken);
                ServiceClientTracing.Enter(_invocationId, this, "GetSourceClientId", tracingParameters);
            }
            // Construct URL
            var _baseUrl = BaseUri.AbsoluteUri;
            var _url = new System.Uri(new System.Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "api/PaymentUrlData/SourceClientId").ToString();
            List<string> _queryParameters = new List<string>();
            if (walletId != null)
            {
                _queryParameters.Add(string.Format("walletId={0}", System.Uri.EscapeDataString(walletId)));
            ...
            if ((int)_statusCode == 200)
            {
                ...
                try
                {
                    _result.Body = SafeJsonConvert.DeserializeObject<SourceClientInfoResponse>(_responseContent, DeserializationSettings);
                }
                catch (JsonException ex)
                {
                    _httpRequest.Dispose();
